...
export default function Navbar ({logoKarenta, handleSidebar}) {
    ...
    return (
        <div className={`navbar fixed ${visible ? 'flex' : 'hidden'} mx-auto z-1000 w-full bg-white border-b-[0.2px] border-[#0D3B8C] shadow-lg px-8 md:px-6 py-2 justify-between md:justify-around lg:gap-18 min-[1920px]:justify-around items-center`}>
            <img src={logoKarenta} alt="" className="w-[136px] h-[51px] 2xl:w-[200px] 2xl:h-[70px] min-[1920px]:-translate-x-18" />

            <ul className="hidden md:flex gap-4 lg:gap-4">
                {navbarMenu.map(menu => <li className="text-[#111928] cursor-pointer hover:text-[#FDC400] 2xl:text-2xl" key={menu}>{menu}</li>)}
            </ul>
            ...
            <RxHamburgerMenu  className="w-[42px] md:hidden scale-200 active:scale-190 text-[#0D3B8C]" onClick={() => handleSidebar()}/>
        </div>
    )
}